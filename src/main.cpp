...
enum {
	...
	SP_ARG_SHOW_COUNTER,
	SP_ARG_CREATE_JPEG,
	SP_ARG_TRY_ORIGIN_JPEG_SP,
	SP_ARG_MERGE_JPEG,
	SP_ARG_MAPPING_OFF,
	SP_ARG_MERGE_MASK,
	SP_ARG_FONT_POSTFIX,
	SP_ARG_FONT_EXCLUDE_CHARS,
	SP_ARG_DEFAULT_FONT,
...
static gchar *sp_font_exclude_chars = NULL;
...
struct poptOption options[] = {
	...
	{"jpeg", 0,
	  ...
	  NULL},
	{"tryOrigJpeg", 0,
	  POPT_ARG_NONE, &sp_try_origin_jpeg_sp, SP_ARG_TRY_ORIGIN_JPEG_SP,
	  N_("Try extract original JPEG stream from PDF instead PNG render. Work if image do not has addition filtres"),
	  NULL},
	{"preserve-dpi", 0,
	  POPT_ARG_NONE, &sp_preserve_dpi_sp, SP_ARG_TRY_ORIGIN_JPEG_SP,
	  N_("Try save original PNG and preserve DPI for hight resolution images"),
	  NULL},
	{"merge-mask-clean", 0,
	  POPT_ARG_NONE, &sp_merge_mask_clean_sp, 0,
	  N_("Conver masks to images one by one. Not try merge objects together"),
	  NULL},
	{"max-difficult", 0,
	  POPT_ARG_INT, &sp_max_difficult_sp, 0,
	  N_("If pdf file contits more than X commands - it will converted to solid image"),
	  N_("NUMBER")},
    {"mergeToJpeg", 0,
	...
	{"fontPostfix", 0,
	 ...
	 NULL},
    {"fontnameExcludeChars", 0,
     POPT_ARG_STRING, &sp_font_exclude_chars, SP_ARG_FONT_EXCLUDE_CHARS,
	 N_("Save exclude char list to the preference.xml. THis list will be removed from font name"),
	 NULL},
	{"defaultFont", 0,
...
sp_process_args(poptContext ctx)
{
    ...
    while ((a = poptGetNextOpt(ctx)) != -1) {
        switch (a) {
            ...
            case SP_ARG_FONT_EXCLUDE_CHARS: {
            	Inkscape::Preferences *prefs = Inkscape::Preferences::get();
            	prefs->setString("/options/svgoutput/fontnameexcludechars", sp_font_exclude_chars);
            	prefs->save();
            	break;
            }
            ...
            case SP_ARG_THUMB_WIDTH: {
            	sp_thumb_width_sh = sp_thumb_width;
            	break;
            }
